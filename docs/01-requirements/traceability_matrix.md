...
| FR-10 | Visitor messages forwarded to admin | FR-10 | WebSocket message routing | `server/app/services/wsconnection_service.py`; connected admin required | TC-010 | Specified; implementation evidence present; test not executed |
| FR-11 | Admin can take over a visitor conversation | FR-11 | Admin WebSocket routing and conversation state | `wsconnection_service.py` (`send_to_user`) | TC-011, TC-021 | Direct messaging is evidenced; takeover state and AI-routing bypass are not implemented; TC-021 not executed |
